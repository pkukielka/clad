...
void THttpServer::parseHeader(char* header) {
  ...
  uint32_t sz = (uint32_t) (colon - header);
  char* value = colon+1;
  ...
  if (strncmp(header, "Transfer-Encoding", sz) == 0) {
    ...
    chunked_ = false;
    contentLength_ = (uint32_t) atoi(value);
  }
}
...
bool THttpServer::parseStatusLine(char* status) {
  ...
  *http = '\0';
  ...
  if (strcmp(method, "POST") == 0) {
    ...
    // preflight OPTIONS method ok, we don't need further content.
    ...
    transport_->write((const uint8_t*)header.c_str(), (uint32_t) header.size());
    ...
    readHeaders_ = true;
    // flush();
    // throw TTransportException(string("We got preflight message."));
    return true;
  }
